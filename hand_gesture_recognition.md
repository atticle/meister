...
*이 교재는 공중 손동작 인터페이스 구현을 완전히 이해하는 것을 목표로 작성되었다.*  
*각 장의 예제 코드를 직접 타이핑하고 실행하면서 익히는 것을 강력히 권장한다.*
## MediaPipe 랜드마크 데이터 구조의 본질

MediaPipe의 핵심 모듈(Hands, Face Mesh, Pose)은 이미지 내에서 객체를 탐지한 후, 정해진 개수의 특징점(Landmark)을 3차원 좌표($x, y, z$) 배열로 반환합니다.

### 정규화된 좌표계 (Normalized Coordinates)

출력되는 $x$와 $y$ 값은 이미지의 실제 픽셀 크기와 관계없이 $0.0$에서 $1.0$ 사이의 상대적인 값을 가집니다.
- $(x: 0.0, y: 0.0)$: 이미지의 좌측 상단 (Top-Left)
- $(x: 1.0, y: 1.0)$: 이미지의 우측 하단 (Bottom-Right)

### $z$ 좌표의 비밀 (Relative Depth)

많은 학습자가 오해하는 부분입니다. MediaPipe의 $z$ 좌표는 절대적인 카메라와의 거리(Depth)가 아닙니다.
- 손(Hands): 손목($\text{Landmark } 0$)을 원점($z=0$)으로 설정합니다. 손목보다 카메라에 가까워지면 음수($-$), 멀어지면 양수($+$) 값을 가집니다. 주된 용도는 손가락이 앞으로 구부러졌는지, 뒤로 젖혀졌는지 판단하는 상대적 깊이 파악입니다.
- 스케일링: $z$ 좌표의 스케일은 $x$ 좌표의 스케일(손의 대략적인 너비)과 유사하게 정규화됩니다.

## 모듈별 고유 랜드마크 체계

프트웨어 응용을 위해 반드시 외우거나 직관적으로 이해해야 하는 핵심 인덱스 맵입니다.

### Hand Landmarks (총 21개 포인트)

손은 관절의 꺾임과 끝점의 위치가 제어의 핵심입니다.

- 인덱스 규칙 (0번~20번):
  - 0: 손목 (Wrist)
  - 4: 엄지 끝 (Thumb Tip)
  - 8: 검지 끝 (Index Finger Tip)
  - 12: 중지 끝 (Middle Finger Tip)
  - 16: 약지 끝 (Ring Finger Tip)
  - 20: 새끼 끝 (Pinky Tip)
- 구조적 특징: 각 손가락은 4개의 포인트로 구성되며, 4의 배수가 각 손가락의 '끝점(Tip)'을 의미합니다.

### Face Mesh Landmarks (총 468개 또는 478개 포인트)

얼굴의 미세한 표정, 눈 깜빡임, 시선 추적을 위해 밀집된 매시 구조를 제공합니다. 센서 제어에 자주 쓰이는 핵심 랜드마크 그룹은 다음과 같습니다.

- 입술 윤곽 (Lips): 61, 146, 91, 181, 84, 17, 314, 405, 321, 375, 291, 308 (입의 개폐 상태 확인 가능)
- 왼쪽 눈 (Left Eye): 362, 382, 381, 380, 374, 373, 390, 249, 263, 466, 388, 387, 386, 385, 384, 398
- 오른쪽 눈 (Right Eye): 33, 7, 163, 144, 145, 153, 154, 155, 133, 173, 157, 158, 159, 160, 161, 246
- 홍채 (Iris, 478개 모델 사용 시): 468~472 (오른쪽 홍채), 473~477 (왼쪽 홍채) $\rightarrow$ 시선 제어(Gaze Tracking)의 핵심.

## 랜드마크 응용을 위한 기반 수학 및 알고리즘

카메라 영상에서 추출한 단순 원시 좌표(Raw Data)를 고차원의 '제스처 명령'으로 변환하기 위한 필수 기법입니다.

### 픽셀 복원 (Pixel Reconstruction)

화면에 사각형을 그리거나 특정 위치에 UI를 매핑하려면 정규화 좌표를 정수형 픽셀 좌표로 환산해야 합니다.

$$\text{Pixel } X = \text{int}(\text{landmark.x} \times \text{Image Width})$$
$$\text{Pixel } Y = \text{int}(\text{landmark.y} \times \text{Image Height})$$

### 유클리드 거리 (Euclidean Distance / L2 Norm)

두 랜드마크 사이의 거리를 측정하여 "손가락을 접었는지 펼쳤는지", "눈을 감았는지" 등을 판별합니다.
예를 들어 검지 끝(8)과 엄지 끝(4)이 만났을 때(Pinch 제스처)를 감지하는 공식입니다.

$$d = \sqrt{(x_2 - x_1)^2 + (y_2 - y_1)^2 + (z_2 - z_1)^2}$$

- 실무 팁: 카메라와 사용자 사이의 거리가 바뀌면 픽셀 거리가 변합니다. 따라서 항상 손목과 중지 기저 관절(0번과 9번) 사이의 거리를 기준 단위(1.0)로 잡고, 다른 거리들을 이에 대해 정규화(Normalization)하여 사용해야 거리 변화에 강인한 시스템이 됩니다.

### 벡터 내적을 이용한 각도 계산 (Angle Calculation)

관절의 구부러진 각도를 계산하여 정밀한 제어 명령을 내릴 때 사용합니다. 세 점 $A, B, C$가 이루는 사잇각 $\theta$는 두 벡터 $\vec{BA}$와 $\vec{BC}$의 내적(Dot Product)을 통해 구합니다.
...
def calculate_angle(a, b, c):
    a = np.array(a) # 예: 손가락 끝
    b = np.array(b) # 예: 중간 관절 (원점)
    c = np.array(c) # 예: 기저 관절
...
### 종횡비 분석 (Aspect Ratio) - 눈 깜빡임 감지

Face Mesh를 활용한 센서 제어에서 가장 많이 쓰이는 기법인 EAR (Eye Aspect Ratio)입니다. 눈의 세로 길이 특징점들과 가로 길이 특징점들의 비율을 계산하여 눈의 개폐 여부를 $0.0 \sim 1.0$ 사이의 수치로 파악합니다

$$EAR = \frac{\|P_2 - P_6\| + \|P_3 - P_5\|}{2 \|P_1 - P_4\|}$$

이 값이 특정 임계값(예: $0.2$) 이하로 떨어지면 사용자가 눈을 감은 것으로 판단하여 '스마트 스위치 ON' 등의 이벤트를 트리거할 수 있습니다.

# 통합 미디어 시스템 아키텍처

최소 i5 랩탑 환경에서 자원 소모를 최소화하고, UI 멈춤(Blocking) 없이 미디어파이프 추론 데이터와 외부 통신을 결합하는 프레임워크 설계를 목료로한다.

## Multi-Threading & IPC 아키텍처 디자인

단일 스레드의 한계: OpenCV 프레임 캡처, MediaPipe 추론, PySide6 UI 렌더링을 하나의 스레드에서 처리하면 FPS가 $10 \sim 15$ 이하로 급감하며 UI가 얼어버리는 현상이 발생합니다.

- Main Thread: PySide6 UI 렌더링 및 사용자 이벤트 루프 관리.
- Worker Thread (QThread): OpenCV 카메라 캡처 + MediaPipe 실시간 추론 연산 전담.
- Message Broker (NanoMQ): 경량 MQTT 브로커를 로컬에 구동하여, Worker Thread가 발행한 제스처 이벤트를 하드웨어 제어 모듈이나 타 서비스가 비동기적으로 구독(Subscribe)하도록 분리.

## 핵심 구현 프레임워크 (PySide6 + NanoMQ 연동 기본 구조)
...
class VisionWorker(QThread):
    # UI 스레드로 갱신된 영상 이미지를 전달하기 위한 시그널
    ...
    def __init__(self):
        ...
        self.hands = self.mp_hands.Hands(
            static_image_mode=False, # 실시간 영상 스트림 최적화
            max_num_hands=1,
    ...
    def run(self):
        ...
        while cap.isOpened():
            ...
            # MediaPipe 추론을 위해 BGR -> RGB 변환
            rgb_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            results = self.hands.process(rgb_frame)
            
            if results.multi_hand_landmarks:
                for hand_landmarks in results.multi_hand_landmarks:
                    # [핵심] 3교시 제스처 알고리즘이 들어갈 위치
                    gesture = self.analyze_gesture(hand_landmarks)
                    if gesture:
                        # 통신 부하를 줄이기 위해 NanoMQ로 비동기 Event Publish
                        self.client.publish("device/control", gesture)
            
            # PySide6 UI 표시를 위한 영상 포맷 변환
